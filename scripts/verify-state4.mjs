// directly read the transform used for actual rendering
...
console.log('default render:', JSON.stringify(await page.evaluate(() => window.__dshRender)));
await page.evaluate(() => {
  document.querySelector('#scale-seg button[data-scale="fixed"]').click();
});
await page.waitForTimeout(400);
console.log('after clicking fixed:', JSON.stringify(await page.evaluate(() => window.__dshRender)));
await page.evaluate(() => {
  const s = document.querySelectorAll('.pen-card')[0].querySelector('.pen-hole');
  s.value = '39'; s.dispatchEvent(new Event('input'));
});
await page.waitForTimeout(300);
console.log('after setting pen 1 to 39:', JSON.stringify(await page.evaluate(() => ({
